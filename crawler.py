...
def get_all_links(page):
	...
	while(True):
		...
		if url:
			...
			break
	return links
def Look_up(index,keyword):#To find the keyword in the index and return the list of links
	if keyword in index:
...
def compute_ranks(graph):#Computing ranks for a given graph 
	d=0.8
	numloops=10
	ranks={}
	npages=len(graph)
...
ranks=compute_ranks(graph)                                  #Calculates page ranks
